...
def build_recommendation(feat: dict, p_up: float) -> dict:
    BUY_TH   = 0.60
    SELL_TH  = 0.60  # enviamos venta si NO compramos (según petición)

    # Por defecto: si no hay compra, vender
    if p_up >= BUY_TH:
        decision = "buy"
        reason = "Probabilidad alta de subida en ~30 min."
    else:
        decision = "sell"
        reason = "Probabilidad insuficiente de subida; preferible venta."
...
def format_bilingual_message(symbol: str, rec: dict, feat: dict) -> str:
    ...
    if rec["decision"] == "buy":
        es_hdr = f"📈 Predicción {symbol} (M5, +30m)\n\n🇪🇸 ES\n"
        es_body = (
...
def parse_args():
    parser = argparse.ArgumentParser(description="Predicción USDJPY M5 a 30m (envía compra o venta, ES+RU).")
